            ...
            <div className="mt-4 text-center text-sm">
              <button
                ...
                onClick={async () => {
                  ...
                  const { error: rpErr } = await supabase.auth.resetPasswordForEmail(email.trim(), {
                    redirectTo: `${window.location.origin}/reset-password`,
                  });
                  if (rpErr) setError(rpErr.message);
                  else setError("Password reset link sent. Check your inbox.");
                }}
                className="text-muted-foreground hover:text-foreground bg-transparent border-none cursor-pointer underline-offset-2 hover:underline"
              >
                Forgot password?
              </button>
            </div>
