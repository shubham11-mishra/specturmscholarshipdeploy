...
const Copilot = () => {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [profileRes, wheelRes, progRes, shortRes, activitiesRes, shortlistedSchoolsRes, userInterestsRes] = await Promise.all([
        ...
        supabase.from("shortlisted_schools").select("school_id,created_at").eq("user_id", user.id),
        supabase.from("user_interests").select("category").eq("user_id", user.id),
      ]);

      const profile = profileRes.data;
      const interests = userInterestsRes.data?.map(i => i.category) ?? [];
      ...
      let scholarshipsQuery = copilotSupabase
        .from("scholarships")
        .select("id,school_name,program_name,category,sub_type,value_aud,value_num,value_type,year_levels,application_open_date,application_close_date,days_left,closing_label,state,suburb,sector,school_type,eligibility_criteria,overview,description,scholarship_url,scholarship_confidence,gender_eligibility,number_awarded,test_month,test_provider,special_conditions,contact_email,contact_phone")
        .eq("is_active", "True");
      if (profile?.state) {
      ...
      const sp: StudentProfile = {
        ...
        is_indigenous: (p?.is_indigenous as boolean) ?? false,
        is_rural: (p?.is_rural as boolean) ?? false,
      ...
      const scoredForAI = rawRows
        .map(s => {
          const row: ScholarshipRow = {
            ...
            school_type: (s as unknown as Record<string, unknown>).school_type as string ?? null,
      ...
      // Keep only essential fields per scholarship to minimise payload size.
      const top15 = scoredForAI.slice(0, 15).map(s => ({
        school_name: s.school_name,
        program_name: s.program_name,
        category: s.category,
        sub_type: (s as Record<string, unknown>).sub_type ?? null,
        match_score: s.match_score,
        match_reasons: s.match_reasons,
        match_tier: s.match_tier,
        value_aud: s.value_aud,
        value_type: (s as Record<string, unknown>).value_type ?? null,
        year_levels: s.year_levels,
        state: s.state,
        suburb: (s as Record<string, unknown>).suburb ?? null,
        sector: (s as Record<string, unknown>).sector ?? null,
        closing_label: s.closing_label,
        application_close_date: s.application_close_date,
        days_left: s.days_left,
        number_awarded: (s as Record<string, unknown>).number_awarded ?? null,
        test_month: (s as Record<string, unknown>).test_month ?? null,
        test_provider: (s as Record<string, unknown>).test_provider ?? null,
        overview: trim((s as Record<string, unknown>).overview),
        eligibility_criteria: trim((s as Record<string, unknown>).eligibility_criteria),
        special_conditions: trim((s as Record<string, unknown>).special_conditions),
        contact_email: (s as Record<string, unknown>).contact_email ?? null,
        contact_phone: (s as Record<string, unknown>).contact_phone ?? null,
      ...
      setContext({
        ...
        current_school: p?.current_school_name ?? null,
        current_school_type: p?.current_school_type ?? null,
        dream_schools: p?.dream_schools ?? null,
        preferred_sectors: p?.preferred_sectors ?? [],
        willing_to_board: p?.willing_to_board ?? null,
        has_sibling_enrolled: p?.has_sibling_enrolled ?? false,
        is_indigenous: p?.is_indigenous ?? false,
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        console.error("Copilot error:", res.status, errBody);
          ...
          <>
            ...
            {messages.length <= 1 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {QUICK_PROMPTS.map((p) => (
                  <button key={p} onClick={() => send(p)} className="text-xs px-3 py-1.5 rounded-full bg-secondary hover:bg-primary/15 text-foreground border border-border transition-colors">{p}</button>
                ))}
              </div>
            )}
            <form onSubmit={(e) => { e.preventDefault(); send(input); }} className="mt-3 flex gap-2">
              <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Copilot anything…" disabled={sending}
