...
export function calculateMatch(student: StudentProfile, s: ScholarshipRow): MatchResult {
  ...
  // Preferred sector match (Catholic, Independent, Government, Anglican)
  if (student.preferred_sectors?.length) {
    const scholarshipSector = ((s.sector || "") + " " + (s.school_sector || "")).toLowerCase();
    const sectorMatch = student.preferred_sectors.some(ps =>
      ps && scholarshipSector.includes(ps.toLowerCase())
    );
    const isOpenToAll = student.preferred_sectors.some(ps =>
      ps?.toLowerCase() === "open_to_all"
    );
    if (isOpenToAll) {
      score += 3;
    } else if (sectorMatch) {
      score += 10;
      const matched = student.preferred_sectors.find(ps => ps && scholarshipSector.includes(ps.toLowerCase()));
      reasons.push(`Matches your preferred sector (${matched})`);
    } else if (scholarshipSector.trim()) {
      score -= 5;
    }
  }
